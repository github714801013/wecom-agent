...
不要输出过长调用链。
默认最多 10 条边。
call_chain 是压缩后防止偏离用户问题的“关系索引”，必须说明节点之间为什么有关联；如果检索结果中能看出“接口 A 调用 B，B 调用 C”，必须至少保留 A->B、B->C 两条边。
如果关系来自代码片段、工具证据或历史摘要，只保留与当前 user_question 直接相关的边；不要把候选但已排除、无关项目或无关历史放入 call_chain。
当压缩预算不足时，优先保留 call_chain、key_evidence 和每个关键节点的 file_path/symbol，再压缩代码正文。
compressed_sections 中的 lines 是后续“已分析代码范围索引”的来源。只要工具结果、代码片段、历史摘要或上下文中能确定行号范围，必须填写稳定范围，例如 "300-500"；无法确认时才留空。
如果历史或输入中已经出现“方法A:300~500”“文件A.java:100-180”“已读取第 510-570 行”等已分析范围，压缩时必须保留这些范围信息，帮助后续分析避开相同位置的重复读取。只有用户要求重新核对该范围内的具体实现时，才建议再次读取同一范围。
...
5. compressed_sections 中每项必须包含：
    ...
    - content
    - merged_from
    其中 lines 必须优先使用工具结果中的真实行号范围，格式统一为 "start-end"；如果当前上下文只知道“方法A:300~500”或“已读取第 510-570 行”，也要转换为该字段，避免后续重复读取同一代码范围。
