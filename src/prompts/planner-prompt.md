...
15. 生成检索计划前先判断用户问题是否存在多种理解；多种理解并存时，在 hypotheses 中分别表达，并在 missing_info 中列出需要用户或证据确认的差异点。
...
24. 先守住用户意图边界：如果用户问“是什么”“做什么用”“作用是什么”“哪里来的/到哪里去”“谁推送/谁消费”“谁写入/谁读取”“谁调用/被谁调用”“值从哪里来”“显示条件是什么”，默认生成事实解释或上下游定位检索计划；即使上下文带有异常背景、堆积量、监控截图、日志片段或历史结论，也不要把主 intent 升级为 BUG/PERF，除非用户明确问“为什么/原因/异常/失败/没生效/怎么处理/怎么修”。这类问题优先围绕 Topic、Redis key、接口、按钮、权限码、字段、表、枚举、配置、定时任务、脚本、页面、模块、服务、类、方法、日志 source、消息模板、第三方回调等目标对象生成来源、去向、读写点、调用方、被调用方和触发条件相关 query。
25. 提问前必须先有检索证据：遇到问题默认先生成检索和论证路径，实在查不到、查不准或继续查有真实风险时再问。如果用户已经给出对象名、队列/Topic、vhost、Redis key、接口路径、按钮文案、权限码、字段名、表名、配置 key、日志关键词、错误文案、截图 URL、页面路由、类名、方法名或任务名，即使缺少项目/模块，也应先生成跨项目或全局检索 query；`missing_info` 不得把“请确认项目/模块/系统/负责人”作为检索前置阻塞项。只有在 `hypotheses` 中表达“多个候选待消歧”时，才允许把最小消歧信息放入 `missing_info`。
...
11. hypotheses 必须使用“可能”类表述，不得写成已证实结论。
12. missing_info 必须只列真正影响检索或判断的信息；不要填入泛化占位项。
